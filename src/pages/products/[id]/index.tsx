...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    setProduct(found);
    if (found) {
      const cat = getCategories().find((c) => c.id === found.categoryId) ?? null;
      setCategory(cat);
    }
    setLoading(false);
  }, [id]);
      ...
      <DashboardLayout title={product.name} description={category?.name ?? ""}>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
              ...
              <Card className="p-4">
                ...
                {youtubeEmbed ? (
                  <div className="aspect-video overflow-hidden rounded-lg bg-black">
                    <iframe src={youtubeEmbed} className="h-full w-full" allowFullScreen />
                  </div>
                ) : (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block rounded-lg border border-border bg-muted p-3 text-sm text-primary hover:underline"
                  >
                    {product.videoUrl}
                  </a>
                )}
          ...
          <div className="space-y-4 lg:col-span-2">
            <Card className="p-6">
              {category && (
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-primary">{category.name}</p>
              )}
              <h1 className="mb-3 text-2xl font-bold">{product.name}</h1>
              <span className={`mb-4 inline-block rounded-md px-2 py-0.5 text-xs font-medium ${statusStyles[product.status]}`}>
